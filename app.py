...
app.config['MYSQL_PASSWORD'] = 'Divyesh@2019'
...
# Initialize Razorpay client after app configuration
razorpay_client = razorpay.Client(auth=(app.config['RAZORPAY_KEY_ID'], app.config['RAZORPAY_KEY_SECRET']))

#Set Current Price of ICOin in rupees
current_price= 127
...
#     balance = get_balance(session.get('username'))
#     coin_amount=form.amount.data
#     if request.method == 'POST':
#         #attempt to buy amount
#         try:
#             send_money("BANK", session.get('username'), coin_amount)
...
def buy():
    ...
    if request.method == 'POST':
        try:
            # Convert form.amount.data from string to integer
            amount = int(form.amount.data)
            currency = "INR"
            receipt = f"order_rcptid_{session.get('username')}"

            # Create order in Razorpay
            razorpay_order = razorpay_client.order.create({
                "amount": amount * 100 * current_price,  # Amount in paise
                "currency": currency,
                "receipt": receipt,
                "payment_capture": '1'
            })
            ...
            return render_template('razorpay_checkout.html', order_id=order_id, order_amount=order_amount, key_id=app.config['RAZORPAY_KEY_ID'])
...
def payment_success():
    ...
    try:
        ...
        coin_amount=(razorpay_client.payment.fetch(payment_id)['amount'])/(100*current_price)
        # amount=str(coin_amount)
        # Proceed with the business logic (e.g., credit coins to user)
        send_money("BANK", session.get('username'),coin_amount)
        flash("Purchase Successful!", "success")
